...
function updateDateTime() {
  ...
  document.getElementById("datetime").textContent =
    `${dayName}, ${date} ${month} ${year} (${hh}:${mm}:${ss} WIB)`;
}
...
if (hashInput && hashOutput) {
  hashInput.addEventListener("input", async (e) => {
    hashOutput.textContent = await sha256(e.target.value);
  });
}
...
// ======== Kirim Transaksi ========
["A", "B", "C"].forEach((u) => {
  const sendBtn = document.getElementById("send-" + u);
  if (sendBtn) {
    sendBtn.onclick = () => {
      ...
      if (mempool) mempool.value = txPool.join("\n");
    };
  }
});
...
// ================== GOOGLE MAPS - KAMPUS PENS ==================
function initGoogleMaps() {
  const gmapsIframe = document.getElementById("gmaps");
  if (!gmapsIframe) return;

  // Koordinat Pascasarjana Terapan Politeknik Elektronika Negeri Surabaya
  // Sesuai marker di Google Maps gambar
  const lat = -7.2885;
  const lon = 112.7968;

  // Buat URL Google Maps embed dengan marker di Pascasarjana Terapan PENS
  const mapsUrl = `https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3956.82!2d${lon}!3d${lat}!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1sPascasarjana%20Terapan%20Politeknik%20Elektronika%20Negeri%20Surabaya!2s-7.2885,112.7968!5e0!3m2!1sid!2sid!4v1700000000000`;
  gmapsIframe.src = mapsUrl;
  console.log("Peta Pascasarjana Terapan PENS dimuat");
}
